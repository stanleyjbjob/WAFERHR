<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Set.aspx.cs" Inherits="Booking_Set"  validateRequest="false"%>
        ...
        <asp:UpdatePanel ID="upl" runat="server">
            <ContentTemplate>
                            ...
                            <tr>
                                ...
                                <td colspan="3">
                                    ...
                                    <asp:CheckBox ID="bAgentAppCheckBox" runat="server" Checked='<%# Bind("bAgentApp") %>'
                                        Text="可幫部屬代理申請" /><asp:CheckBox ID="CheckBox2" runat="server" Checked='<%# Bind("bSignState") %>' Text="顯示進行中流程" /><asp:CheckBox ID="bNoteCheckBox" runat="server" Checked='<%# Bind("bNote") %>' Text="事由一定要填" /></td>
                            </tr>
                            <tr>
                                <td class="UltraWebGrid1-hc" nowrap="nowrap" style="text-align: right" width="1%">
                                    最大申請筆數</td>
                ...
                <asp:SqlDataSource ID="sdsSet" runat="server" ConnectionString="<%$ ConnectionStrings:flow %>"
                    DeleteCommand="DELETE FROM [wfBookingSet] WHERE [iAutoKey] = @iAutoKey" InsertCommand="INSERT INTO [wfBookingSet] ([sStdNote], [sCheckNote], [bSignNote], [bAgentApp], [bSignState], [bNote], [iAppCount], [sKeyMan], [dKeyDate]) VALUES (@sStdNote, @sCheckNote, @bSignNote, @bAgentApp, @bSignState, @bNote, @iAppCount, @sKeyMan, @dKeyDate)"
                    SelectCommand="SELECT * FROM [wfBookingSet]" UpdateCommand="UPDATE [wfBookingSet] SET [sStdNote] = @sStdNote, [sCheckNote] = @sCheckNote, [bSignNote] = @bSignNote, [bAgentApp] = @bAgentApp, [bSignState] = @bSignState, [bNote] = @bNote, [iAppCount] = @iAppCount, [sKeyMan] = @sKeyMan, [dKeyDate] = @dKeyDate WHERE [iAutoKey] = @iAutoKey">
                    <DeleteParameters>
