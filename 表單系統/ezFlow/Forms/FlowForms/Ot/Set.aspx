<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Set.aspx.cs" Inherits="Ot_Set" validateRequest="false"%>
        ...
        <asp:UpdatePanel ID="upl" runat="server">
            <ContentTemplate>
                            ...
                            <tr>
                                ...
                                <td colspan="3">
                                    <asp:CheckBox ID="CheckBox1" runat="server" Checked='<%# Bind("bSignNote") %>' Text="顯示主管簽核意見" /><asp:CheckBox ID="bAgentAppCheckBox" runat="server" Checked='<%# Bind("bAgentApp") %>'
                                        Text="可幫部屬代理申請" /><asp:CheckBox ID="CheckBox2" runat="server" Checked='<%# Bind("bSignState") %>' Text="顯示進行中流程" /><asp:CheckBox ID="bAttendCheckBox" runat="server" Checked='<%# Bind("bAttend") %>'
                                        Text="需要判斷出勤" /><asp:CheckBox ID="bNoteCheckBox" runat="server" Checked='<%# Bind("bNote") %>' Text="事由一定要填" /></td>
                            </tr>
                            <tr>
                                <td class="UltraWebGrid1-hc" nowrap="nowrap" style="text-align: right" width="1%">
                                    最大申請筆數</td>
                ...
                <asp:SqlDataSource ID="sdsSet" runat="server" ConnectionString="<%$ ConnectionStrings:flow %>"
                    DeleteCommand="DELETE FROM [wfOtSet] WHERE [iAutoKey] = @iAutoKey" InsertCommand="INSERT INTO [wfOtSet] ([sStdNote], [sCheckNote], [bSignNote], [bAgentApp], [bSignState], [bAttend], [bNote], [iAppCount], [iDelay], [sKeyMan], [dKeyDate]) VALUES (@sStdNote, @sCheckNote, @bSignNote, @bAgentApp, @bSignState, @bAttend, @bNote, @iAppCount, @iDelay, @sKeyMan, @dKeyDate)"
                    SelectCommand="SELECT * FROM [wfOtSet]" UpdateCommand="UPDATE [wfOtSet] SET [sStdNote] = @sStdNote, [sCheckNote] = @sCheckNote, [bSignNote] = @bSignNote, [bAgentApp] = @bAgentApp, [bSignState] = @bSignState, [bAttend] = @bAttend, [bNote] = @bNote, [iAppCount] = @iAppCount, [iDelay] = @iDelay, [sKeyMan] = @sKeyMan, [dKeyDate] = @dKeyDate WHERE [iAutoKey] = @iAutoKey">
                    <DeleteParameters>
